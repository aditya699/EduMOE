...
3. Updated learning rate scheduler to use cosine decay after warmup (as suggested by medailabs manager)
...
class TrainingManager:
    ...
    def _init_wandb(self):
        ...
        wandb.init(
            ...
            config={
                ...
                # Match the model config which uses block_size as context window
    ...
    def _create_model_config(self):
        ...
        return SimpleNamespace(
            vocab_size=self.config.vocab_size,
            # Match context window to dataset block size to avoid indexing errors
            max_position_embeddings=self.config.block_size,
            n_embd=self.config.n_embd,
            n_layer=self.config.n_layer,
            n_head=self.config.n_head,
